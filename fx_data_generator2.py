...
AMOUNT_BINS = [
    (0, 100_000),(100_000, 400_000),(400_000, 1_500_000),(1_500_000, 3_000_000),
    (3_000_000, 6_000_000),(6_000_000, 11_000_000),(11_000_000, 20_000_000),
    (20_000_000, 40_000_000),(40_000_000, 80_000_000),(80_000_000, 220_000_000),
...
def sample_fx_pair():
    if random.random() < 0.8:
        sell = "AMD"; buy = random.choice([c for c in CURRENCIES if c != "AMD"])
    else:
...
def gen_tin() -> str: return str(random.randint(10_000_000, 99_999_999))
...
def pick_exec_environment(buy_exec: str, sell_exec: str, tx_type: str) -> str:
    if is_cash_pair(buy_exec, sell_exec): allowed = ["InPerson", "FGIAS"]
    else: allowed = ["Web", "Mobile", "POS", "InPerson"] if tx_type=="Direct" else ["Web", "Mobile", "POS", "VirtualPOS"]
    return random.choice(allowed)

def round2(x: float) -> float: return float(f"{x:.2f}")
def integer_digits_len(x: float) -> int: return len(str(int(floor(abs(x)))))

rows = []; seq_map = {b["organization_id"]: 1 for b in BANKS}

for _ in range(N_ROWS):
    bank = random.choice(BANKS); org = bank["organization_id"]
    dt = DATE_START + timedelta(days=random.randint(0, DATE_SPAN_DAYS),
                                hours=random.randint(0,23), minutes=random.randint(0,59), seconds=random.randint(0,59))
    signing_date = dt.strftime("%d/%m/%Y"); signing_time = dt.strftime("%H:%M:%S")
    exec_dt = dt + timedelta(minutes=random.randint(0,120))
    execution_date = exec_dt.strftime("%d/%m/%Y"); execution_time = exec_dt.strftime("%H:%M:%S")
    time_period = to_timeperiod_bucket_with_seconds(dt)

    parent_message_id = make_parent_message_id(org, dt, seq_map[org]); seq_map[org]+=1
    ...
    lo, hi = pick_amount_bin(amd_amount); amount_range = f"{int(lo)} – {int(hi)}"
    ...
    partner_code = f"CBA-{random.randint(100000,999999)}" if partner_legal in ["Leg","SEnt"] and random.random()<0.5 else None
    ...
    if partner_legal == "Nat":
        if random.random() < 0.9: partner_ssn = gen_psn_or_certificate(0.8)
        if partner_ssn is None:
            if (amd_amount>=100_000 and tx_type=="Direct" and established) or (account_on_whose=="OnCustBehalf"):
        ...
        if is_leglike:
            if (partner_residency=="Resident" and amd_amount>=100_000 and tx_type=="Direct" and established) or \
               (account_on_whose=="OnCustBehalf" and partner_residency=="Resident"):
                partner_tin = gen_tin()
            elif random.random() < 0.5:
                partner_tin = gen_tin()
            ...
            if need_name1 or need_name2: partner_legalName = f"Ընկերություն {random.randint(1000,9999)}"
            elif random.random() < 0.15: partner_legalName = f"Ընկերություն {random.randint(1000,9999)}"
    ...
    if include_client:
        ...
        if client_legal in ["Leg","SEnt","NA"]:
            if client_residency == "Resident":
                ...
                if random.random() < 0.5: client_tin = gen_tin()
                if client_tin is None: client_legalName = f"Պատվիրատու ՍՊԸ {random.randint(100,999)}"
    ...
    if include_intermediary:
        ...
        intermediary_residency = "Resident" if intermediary_country=="AM" else random.choice(RESIDENCY)
        intermediary_link = random.choice(LINK_FLAG)
        if random.random() < 0.25: intermediary_code = f"CBA-{random.randint(100000,999999)}"
        if random.random() < 0.6: intermediary_tin = gen_tin()
        if intermediary_tin is None: intermediary_legalName = f"Միջնորդ {random.randint(1000,9999)}"

    buy_exec = random.choice(EXEC_METHOD); sell_exec = random.choice(EXEC_METHOD)
    ...
    rows.append({
        "external_id": external_id, "parent_message_id": parent_message_id, "organization_id": org,
        "branchId": f"{random.randint(1,3):03d}",
        "partner_country": partner_country, "partner_residency": partner_residency,
        "partner_legalStatus": partner_legal, "partner_sectOralAffiliation": sectoral,
        "partner_economicBranch": econ_branch, "partner_codeByCBA": partner_code,
        "partner_linkToFinancialInstitution": partner_link,
        "partner_ssn": partner_ssn, "partner_passport": partner_passport,
        "partner_tin": partner_tin, "partner_legalName": partner_legalName,
        "client_country": client_country, "client_residency": client_residency,
        "client_legalStatus": client_legal, "client_sectoralAffiliation": (random.choice(["Financial","Public","Private","Household","NA"]) if include_client else None),
        "client_economicBranch": (random.choice(["G_46","G_47","M_70","NA"]) if include_client else None),
        "client_codeByCBA": client_code, "client_linkToFinancialInstitution": client_link,
        "client_ssn": client_ssn, "client_passport": client_passport,
        "client_tin": client_tin, "client_legalName": client_legalName,
        "intermediary_country": intermediary_country, "intermediary_residency": intermediary_residency,
        "intermediary_codeByCBA": intermediary_code, "intermediary_linkToFinancialInstitution": intermediary_link,
        "intermediary_tin": intermediary_tin, "intermediary_legalName": intermediary_legalName,
        "buyCurrency": buy_cur, "sellCurrency": sell_cur,
        "buyVolume": buy_vol, "sellVolume": sell_vol,
        "exchangeRate": ex_rate, "publishedExchangeRate": pub_rate,
        "amountRange": amount_range, "buyExecutionMethod": buy_exec, "sellExecutionMethod": sell_exec,
        "commissionFee_AMD": commission,
        "accountOnWhoseBehalf": account_on_whose, "nameOnWhoseBehalf": name_on_whose,
        "transactionType": tx_type, "transactionForm": tx_form,
        "transactionSigningDate": signing_date, "transactionExecutionDate": execution_date,
        "SigningTime": signing_time, "ExecutionTime": execution_time, "timePeriod": time_period,
        "transactionSigningPlace": signing_place, "transactionExecutionEnvironment": exec_env,
        "_amdAmount_AMD": amd_amount, "_internal_buy_vol": internal_buy_vol, "_internal_sell_vol": internal_sell_vol,
    })
...
# Columns that must be identical inside a summary group (mapped to your schema)
...
def _env_ok(buy_e: str, sell_e: str, tx_type: str, env: str) -> bool:
    ...
    return env in ({"Web","Mobile","POS","InPerson"} if tx_type == "Direct" else {"Web","Mobile","POS","VirtualPOS"})
...
for gidxs in idx_groups:
    ...
    if tpl["accountOnWhoseBehalf"] == "OnItsBehalf":
        for c in ["client_country","client_residency","client_legalStatus",
...
client_presence_cols = ["client_country","client_residency","client_legalStatus","client_sectoralAffiliation","client_economicBranch","client_codeByCBA","client_linkToFinancialInstitution"]
for c in client_presence_cols: df.loc[~mask_oncust, c] = None

def fill_if_null(series, filler):
    m = series.isna() & mask_oncust
    if m.any(): series.loc[m] = [filler() for _ in range(m.sum())]
    return series
...
df.loc[mask_oncust & mask_client_nat, "client_ssn"] = df.loc[mask_oncust & mask_client_nat, "client_ssn"].where(
    df.loc[mask_oncust & mask_client_nat, "client_ssn"].notna(),
    [f"{random.randint(1_000_000_000, 9_999_999_999)}" for _ in range((mask_oncust & mask_client_nat).sum())]
)
df.loc[~(mask_oncust & mask_client_nat), "client_ssn"] = None
need_client_passport = mask_oncust & mask_client_nat & df["client_ssn"].isna()
df.loc[need_client_passport, "client_passport"] = df.loc[need_client_passport, "client_passport"].where(
...
df.loc[need_client_tin, "client_tin"] = df.loc[need_client_tin, "client_tin"].where(
    df.loc[need_client_tin, "client_tin"].notna(),
    [str(random.randint(10_000_000, 99_999_999)) for _ in range(need_client_tin.sum())]
)
...
df.loc[need_client_name, "client_legalName"] = df.loc[need_client_name, "client_legalName"].where(
    df.loc[need_client_name, "client_legalName"].notna(),
    [f"Պատվիրատու ՍՊԸ {random.randint(100,999)}" for _ in range(need_client_name.sum())]
)
df.loc[mask_oncust & ~mask_leglike_client, "client_legalName"] = None

# Intermediary presence
id_or_link_cols = ["intermediary_tin","intermediary_legalName","intermediary_codeByCBA","intermediary_linkToFinancialInstitution"]
...
df.loc[mask_i_need_name, "intermediary_legalName"] = df.loc[mask_i_need_name, "intermediary_legalName"].where(
    df.loc[mask_i_need_name, "intermediary_legalName"].notna(),
    [f"Միջնորդ {random.randint(1000,9999)}" for _ in range(mask_i_need_name.sum())]
)
...
df.loc[mask_indirect, "transactionForm"] = df.loc[mask_indirect, "transactionForm"].fillna("Փոխանցում")
...
def env_allowed(row) -> bool:
    ...
    if is_cash_pair(buy_e, sell_e): return env in {"InPerson","FGIAS"}
    return env in {"Web","Mobile","POS","InPerson"} if row["transactionType"]=="Direct" else env in {"Web","Mobile","POS","VirtualPOS"}

bad_env_mask = ~df.apply(env_allowed, axis=1)
df.loc[bad_env_mask, "transactionExecutionEnvironment"] = df[bad_env_mask].apply(
    lambda r: pick_exec_environment(r["buyExecutionMethod"], r["sellExecutionMethod"], r["transactionType"]),
    axis=1
)
...
def check_volume_rules(series: pd.Series, name: str):
    ...
    assert s.apply(lambda v: re.fullmatch(r"^\d{1,12}(\.\d{1,2})?$", f"{v:.2f}")).all(), f"{name} must have ≤ 2 decimals"

check_volume_rules(df["buyVolume"], "buyVolume")
check_volume_rules(df["sellVolume"], "sellVolume")

assert (df["exchangeRate"] > 0).all()
if df["publishedExchangeRate"].notna().any(): assert (df["publishedExchangeRate"].dropna() > 0).all()
...
assert df["timePeriod"].str.match(r"^\d{2}:\d{2}:\d{2}\s–\s\d{2}:\d{2}:\d{2}$").all()

def date_ge(a, b): return datetime.strptime(a, "%d/%m/%Y") >= datetime.strptime(b, "%d/%m/%Y")
assert df.apply(lambda r: date_ge(r["transactionExecutionDate"], r["transactionSigningDate"]), axis=1).all()
...
for c in required_cols: assert df[c].notna().all(), f"{c} is required (1..1)"

if mask_partner_mand.any():
    must_cols = ["partner_country","partner_residency","partner_legalStatus","partner_sectOralAffiliation","partner_economicBranch","partner_linkToFinancialInstitution"]
    assert df.loc[mask_partner_mand, must_cols].notna().all().all(), "Partner classifiers must be present when mandatory"

assert (df.loc[mask_plegal_present & mask_nat, "partner_sectOralAffiliation"] == "Household").all()
if df["partner_legalStatus"].notna().any():
    assert df.loc[~df["partner_legalStatus"].isin(["Leg","SEnt"]), "partner_codeByCBA"].isna().all()

# Client identity
assert df.loc[mask_oncust & (df["client_legalStatus"]=="Nat"), "client_ssn"].notna().all()
assert df.loc[~(mask_oncust & (df["client_legalStatus"]=="Nat")), "client_ssn"].isna().all()
...
assert df.loc[mask_oncust & mask_leglike_client2 & mask_client_resident2, "client_tin"].notna().all()
assert df.loc[mask_oncust & ~mask_leglike_client2, "client_tin"].isna().all()
mask_ctin_null = df["client_tin"].isna()
assert df.loc[mask_oncust & mask_leglike_client2 & mask_ctin_null, "client_legalName"].notna().all()
assert df.loc[mask_oncust & ~mask_leglike_client2, "client_legalName"].isna().all()

# Intermediary
assert (df.loc[df[["intermediary_tin","intermediary_legalName","intermediary_codeByCBA","intermediary_linkToFinancialInstitution"]].notna().any(axis=1), ["intermediary_country","intermediary_residency"]].notna().all(axis=1)).all()
assert df.apply(env_allowed, axis=1).all()

df = df.drop(columns=["_amdAmount_AMD","_internal_buy_vol","_internal_sell_vol"])
out_path = "fx_details_test.xlsx"
